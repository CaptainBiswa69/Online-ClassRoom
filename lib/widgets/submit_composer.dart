...
class _SubmissionComposerState extends State<SubmissionComposer> {
  ...
  Widget build(BuildContext context) {
    ...
    Submission userSubmission = submissionList.firstWhere((i) =>
        i.assignment.title == widget.assignment.title &&
        i.user.uid == user.uid &&
        i.classroom.className == widget.assignment.classroom.className);
      ...
      children: [
        ...
        if (attachments.isNotEmpty)
          Padding(
              ...
              child: AttachmentEditorComposer(
                  attachmentList: attachments,
                  isTeacher: false,
                  title: widget.assignment.title,
                  className: widget.assignment.classroom.className)),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: OutlinedButton(
              onPressed: () {
                addFile(account!.email as String,
                    widget.assignment.classroom.className);
                setState(() => {});
              },
              child: Padding(
                  padding:
            ...
            child: MaterialButton(
              ...
              onPressed: () async {
                for (int i = 0; i < attachments.length; i++) {
                  String safeURL =
                      attachments[i].url.replaceAll(RegExp(r'[^\w\s]+'), '');

                  await AttachmentsDB().createAttachmentStudentsDB(
                      user.uid,
                      attachments[i].url,
                      safeURL,
                      "${widget.assignment.classroom.className}__${widget.assignment.title}");
                }
