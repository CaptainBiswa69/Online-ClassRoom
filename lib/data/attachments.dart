...
class AttachmentStudents {
  ...
  AttachmentStudents(
      {required this.uid, required this.submission, required this.url});
}
...
Future<bool> getAttachAnnounceList() async {
  ...
  for (var element in jsonList) {
    var data = element.data();
    attachAnnounceList.add(AttachmentAnnouncement(
      title: data["title"],
      url: data["url"],
    ));
  }
...
Future<bool> getAttachmentStudentsList() async {
  ...
  jsonList.forEach((element) {
    var data = element.data();
    attachmentStudentsList.add(AttachmentStudents(
      uid: data["uid"],
      url: data["url"],
...
Attachment? getAttachment(url) {
  var data = attachmentList.firstWhere((element) => element.url == url,
      orElse: () => null);
...
List getAttachmentListForAnnouncement(title) {
  List list1 =
      attachAnnounceList.where((element) => element.title == title).toList();
  List required = [];
...
List getAttachmentListForStudent(uid, classname, submission) {
  List list1 = attachmentStudentsList
      .where((element) =>
          element.uid == uid &&
          element.submission == classname + "__" + submission)
      .toList();
  List required = [];
  list1.forEach((element) {
    required.add(getAttachment(element.url)!);
